...
/**
 * Loads the timecode JSON from the root directory based on the current language and starts monitoring the current audio.
 */
export async function initializeWordByWordHighlighter() {

  return;

  const timecodeJsonUrl = `./timecode_output_${state.currentLanguage}.json`;
  try {
  ...
  if (state.easyReadMode) {
    ...
    if (!isHeader && !isExcluded) {
      ...
      if (state.translations && state.translations.hasOwnProperty(easyReadKey)) {
        ...
        // Log which timecode data we're using
        if (timecodeKey !== easyReadKey) {
          console.log(`Using fallback timecode data for ${easyReadKey} -> ${dataId}`);
        }
      }
    }
  }
...
// Revised handleClickOutside function
...
// Add this function to position the subtitle popup
...
// Update hideSubtitlePopup to be more robust
...
// Modify the original clearHighlights function to also clean up subtitles
