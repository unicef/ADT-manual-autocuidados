...
export const clearInputValidationFeedback = (input) => {
    ...
    const inputId = input.id || ''; // Add this line to define inputId
    ...
    // const feedbackIcon = document.querySelector(`.feedback-icon-for-${dataActivityItem}`);
    // if (feedbackIcon) {
    //     feedbackIcon.remove();
    // }

    // // Also check for icons linked by ID
    // const idBasedIcon = document.querySelector(`.feedback-icon-for-${inputId}`);
    // if (idBasedIcon) {
    //     idBasedIcon.remove();
    // }

    // // Also check for profanity and gibberish icons
    // const profanityIcon = document.querySelector(`.feedback-icon-for-${input.id || 'profanity'}`);
    // const gibberishIcon = document.querySelector(`.feedback-icon-for-${input.id || 'gibberish'}`);

    // if (profanityIcon) profanityIcon.remove();
    // if (gibberishIcon) gibberishIcon.remove();
    ...
    // We should keep the original aria-label if it existed, but remove validation text
    const originalLabel = input.getAttribute("aria-label") || "";
    if (originalLabel.includes(" - ")) {
        // If the aria-label has validation text (contains " - "), remove it
    ...
    // // Clear the clearing state
    // setTimeout(() => {
    //     input.removeAttribute('data-clearing-feedback');
    // }, 50);
}
