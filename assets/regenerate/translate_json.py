...
def main(input_path: str, output_path: str, num_workers: int = 6):
    ...
    print(
        f"Found {total} strings to translate. Starting parallel translation with {num_workers} threads..."
    )
    ...
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        futures = [
            executor.submit(translate_and_print, idx, path, text)
            for idx, (path, text) in enumerate(string_paths)
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", "-o", required=True, help="Path to output (translated) JSON file"
    )
    parser.add_argument(
        "--threads", "-t", type=int, default=6, help="Number of parallel threads (default: 6)"
    )
